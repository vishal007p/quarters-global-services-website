...
const DropdownWrapper = ({
  ...
  return (
    <div className="w-full md:w-1/3 flex flex-col">
      <DropdownMenu>
        <DropdownMenuTrigger
          className={`px-4 py-2 rounded-md shadow-md text-left flex items-center gap-2 ${
            errors ? "border border-red-500" : "border border-gray-600"
          } bg-black text-white`}
        >
          {/* show selected value */}
        ...
        <DropdownMenuContent className="bg-[#1a1a1a] rounded-md shadow-lg p-2 w-full min-w-[300px]">
          ...
          <div className="max-h-60 overflow-y-auto">
            {filteredOptions.map((option: any) => (
              ...
              >
                {type === "flag" && (
                  <Flags code={option.code} style={{ width: 20, height: 15 }} />
                )}
                {option.name}
              </DropdownMenuItem>
            ))}
